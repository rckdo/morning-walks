...
- **Three ways off the pad**, and the middle one matters: `ingest` promotes an item into an observation, `removeScratch` bins a single item, `clearScratch` empties the lot. Without `removeScratch` (added v120) the only way to shift one piece of junk sitting among good items was to ingest it somewhere it didn't belong — which quietly corrupts the record the pad exists to protect. Note the scratch id prefix is **`x`** (`x1`, `x2`), not `s` — `s` ids are solutions.
- **The pad is styled cool against the record's warm stock** — a grey wash, in `#tier2`-scoped rules that set colour and nothing else. Uncommitted material must not read as part of the record at a glance, and it must not read as a warning either: nothing on the pad is urgent, it simply isn't banked yet.
...
- **Batches are atomic (v122).** One invalid op refuses the whole batch — nothing half-applies. `dryRun:true` validates and reports without writing; `export_review preview:true` returns an included/withheld manifest without freezing anything; `reopen_thread` reopens a closed thread with the prior resolution kept on its `history[]`; threads may carry `supersedes[]` links (a link only — nothing auto-resolves).
